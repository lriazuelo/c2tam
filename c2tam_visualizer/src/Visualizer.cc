...
//  people_remove_sub = nh_.subscribe("/vslam/peopleRemove", 1, &Visualizer::peopleRemoveCb, this);
...
//20072015
/*
void Visualizer::peopleRemoveCb(const c2tam_msgs::PeopleList::ConstPtr& msg)
{
  fprintf(stderr,"peopleRemoveCb %d size dense %d\n",msg->nKF,mpMap->densePoints.size());

  for(int p = 0; p < msg->people.size() ;p++){
  //  int inicio = (640 * (int)(msg->people[p].y - (msg->people[p].size))) + (int)(msg->people[p].x - (msg->people[p].size));
    int inicio = (640 * (int)(msg->people[p].y)) + (int)(msg->people[p].x);
    for(int j = 0; j < msg->people[p].size ; j++){
      for(int i = 0; i < msg->people[p].size ; i++){
        if(inicio+i < 640*480){
          mpMap->densePoints[msg->nKF-1][inicio+i].z = 0;
          mpMap->densePoints[msg->nKF-1][inicio+i].x = 0;
          mpMap->densePoints[msg->nKF-1][inicio+i].y = 0;
        }
      }
      inicio = inicio + 640;
    }
  ...
  //for (int i=96000; i < 192000; i++){
  //  mpMap->densePoints[msg->nKF-1][i].z = 0;
  //  mpMap->densePoints[msg->nKF-1][i].x = 0;
  //  mpMap->densePoints[msg->nKF-1][i].y = 0;
  //}
