...
// which are within radius nRange of the center. (Params are supplied in Level0
// coords.) Returns true on patch found.
bool PatchFinder::FindPatchCoarseTemplateMatching(ImageRef irPos, KeyFrame &kf, unsigned int nRange)
{
  mbFound = false;
  ...
  // Ref variable for the search level
  Level &L = kf.aLevels[mnSearchLevel];
 // cerr<<"el nivel de busqueda "<<mnSearchLevel<<endl;  // Some bounds checks on the bounding box..
  if(nTop < 0)
    nTop = 0;
  if(nTop >= L.im.size().y)
    return false;
  if(nBottomPlusOne <= 0)
    return false;
  //cerr<<"ha pasado los bounds\n";

  ImageRef irBest;             // Best match so far
  int nBestSSD = mnMaxSSD + 1; // Best score so far is beyond the max allowed
  ImageRef ir2;

  //cerr<<"limites "<<nBottomPlusOne<<" "<<nTop<<" "<<nLeft<<" "<<nRight<<endl;
  for (unsigned int i= (int) nTop; i<(int) nBottomPlusOne ;i++) {
      for(unsigned int j= (int) nLeft; j<(int) nRight;j++){
          ir2.x = j;
          ir2.y = i;
          int nZMSSD = ZMSSDAtPoint(L.im, ir2);
          if(nZMSSD < nBestSSD)
          {
              nBestSSD = nZMSSD;
              irBest= ir2;
...
//  if(mbFound){
// // cerr<<"...salvando\n";
//   CVD::Image< CVD::Rgb<CVD::byte> > imTarget = CVD::convert_image(L.im);
//   CVD::drawLine (imTarget, (double) nLeft , (double) nBottomPlusOne, (double) nRight, (double) nBottomPlusOne , CVD::color< CVD::Rgb<CVD::byte> >::red() );
//   CVD::drawLine (imTarget, (double) nRight, (double) nBottomPlusOne, (double) nRight, (double) nTop, CVD::color< CVD::Rgb<CVD::byte> >::red() );
//   CVD::drawLine (imTarget, (double) nRight, (double) nTop , (double) nLeft,  (double) nTop , CVD::color< CVD::Rgb<CVD::byte> >::red() );
//   CVD::drawLine (imTarget, (double) nLeft,  (double) nTop , (double) nLeft,  (double) nBottomPlusOne , CVD::color< CVD::Rgb<CVD::byte> >::red() );
//   CVD::drawLine (imTarget, (double) mv2CoarsePos[0],  (double) mv2CoarsePos[1] , (double) mv2CoarsePos[0]+0.1,  (double) mv2CoarsePos[1]+0.1 , CVD::color< CVD::Rgb<CVD::byte> >::blue() );
//   std::ostringstream filename;
//   filename << "debug/target" << kf.nFrame<<"_"<<irPos.x<<"_"<<irPos.y<< ".png";
//   std::fstream os (filename.str().c_str(), fstream::out | fstream::binary);
//   CVD::img_save< CVD::Rgb<CVD::byte> > (imTarget,os,CVD::ImageType::PNG);
//   os.close();
//  }
 // cerr<<"...done\n";
  return mbFound;
