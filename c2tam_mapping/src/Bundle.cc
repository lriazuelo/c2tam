...
int Bundle::Compute(bool *pbAbortSignal)
{
  ...
  while(!mbConverged  && !mbHitMaxIterations && !*pbAbortSignal)
  {
     ...
     else if(*gvsMEstimator == "Tukey") bNoError = Do_LM_Step<Tukey>(pbAbortSignal);
     else if(*gvsMEstimator == "Huber") bNoError = Do_LM_Step<Huber>(pbAbortSignal);
     else
	  {
  ...
  for(list<Meas>::iterator itr = mMeasList.begin(); itr!=mMeasList.end(); itr++)
    {
      ...
      if(!meas.bBad) vdErrorSquared.push_back(meas.dErrorSquared);
    };
