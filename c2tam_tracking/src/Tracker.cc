             ...
             queueSizeInfo < 3)
        {
         ...
         pthread_mutex_unlock(&addKFMutex); 
          ...
          //  }
            ...
            //  << mMap.vpKeyFrames.size() << "\t" << mMap.vpPoints.size() << "\t" << 1 << "\t" << t2 << endl;
            //  timeStat.flush();
            //}
         };
...
//  if((dDist > GV2.GetDouble("MapMaker.MaxKFDistWiggleMult",1.0,SILENT) * mdWiggleScaleDepthNormalized) ||
  if((dDist > GV2.GetDouble("MapMaker.MaxKFDistWiggleMult",needKFCoefficient,SILENT) * mdWiggleScaleDepthNormalized) ||
    ((dDist < GV2.GetDouble("MapMaker.MaxKFDistWiggleMult",(double) needKFCoefficient/ (double) 3,SILENT) * mdWiggleScaleDepthNormalized) && aDist > 0.5)){
   return true;
  }
  return false;
}
