  ...
  for(int i=0; i<LEVELS; i++)
  {
      ...
      if(i!=0)
	   {  // .. make a half-size image from the previous level..
	      ...
	      // Hacemos un binning 2x2 para reducir la imagen a la 4ª parte
	      // de su superficie
      ...
      if(i == 1)
	      fast_corner_detect_10(lev.im, lev.vCorners, 25);
      if(i == 2)
	      fast_corner_detect_10(lev.im, lev.vCorners, 25);
      if(i == 3)
	      fast_corner_detect_10(lev.im, lev.vCorners, 20);
...
void KeyFrame::MakeKeyFrame_Rest()
{
  ...
  for(int l=0; l<LEVELS; l++)
  {
      ...
      fast_nonmax(lev.im, lev.vCorners, 20, lev.vMaxCorners);
      ...
      {
         if(!lev.im.in_image_with_border(*i, 10)) continue;
...
struct LevelHelpersFiller // Code which should be initialised on init goes here; this runs before main()
{
  LevelHelpersFiller()
  {
    for(int i=0; i<LEVELS*2; i++)
      {
	if(i==0)  gavLevelColors[i] = makeVector( 1.0, 0.0, 0.0); // Rojo
	else if(i==1)  gavLevelColors[i] = makeVector( 1.0, 1.0, 0.0); // Amarillo
	else if(i==2)  gavLevelColors[i] = makeVector( 0.0, 1.0, 0.0); // Verde
	else if(i==3)  gavLevelColors[i] = makeVector( 0.0, 0.0, 0.7); // Azul
	else if(i==4)  gavLevelColors[i] = makeVector( 1.0, 0.6, 0.6); // Rosa
	else if(i==5)  gavLevelColors[i] = makeVector( 1.0, 1.0, 0.6); // Amarillo claro
	else if(i==6)  gavLevelColors[i] = makeVector( 0.6, 1.0, 0.6); // Verde claro
	else if(i==7)  gavLevelColors[i] = makeVector( 0.6, 0.6, 1.0); // Azul claro
